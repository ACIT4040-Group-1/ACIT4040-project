...
def image_quality_measures(refImage, testImage):
    ...
    diffImg = refImage.astype(np.float64) - testImage.astype(np.float64)
    diffSq = np.square(diffImg)
    sumDiffSq = np.sum(diffSq)
    absDiffImg = np.absolute(diffImg)

    refSq = np.square(refImage.astype(np.float64))
    ...
    testSq = np.square(testImage.astype(np.float64))
...
def edge_similarity(refImage, testImage):
    ...
    refSobel_sep = scipy.ndimage.sobel(refImage)
    ...
    testSobel_sep = scipy.ndimage.sobel(testImage)
